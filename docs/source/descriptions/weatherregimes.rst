...
.. code-block:: R
   ...
   library(maps)
load("~/data/tests/output_classification-e49f2d78-8563-11e6-bf14-fbeae168c26e.Rdat")
...
for(i in 1:nreg){ #nreg=4
  ...
  #plotting same information in colors and contours for a better view
  filled.contour(lon,sort(lat),dum[,lat.sort$ix],color.palette =bluered,
                 ...
                 plot.axes={axis(1); axis(2); 
                   contour(lon,sort(lat),dum[,lat.sort$ix],add=T, nlevels=(length(zlev)/2),lwd=2);
                   library(fields)
